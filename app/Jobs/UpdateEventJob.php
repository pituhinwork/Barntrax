...
class UpdateEventJob extends Job implements SelfHandling
{
     ...
     * @param $type_changed
     */
    public function __construct($event, $type, $type_id, $name, $date, $recurring, $icon, $type_changed, $archived)
    {
        $this->event        = $event;
        $this->type         = $type;
    ...
    public function handle(EventRepository $events)
    {
        ...
        if ($event->isRecurring()) {
            if ($event->recurringEvents->isEmpty()) {
                ...
                $date = Carbon::createFromFormat(\Auth::user()->getDateFormatPHP(), $event->date);
