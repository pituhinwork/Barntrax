...
class CreateLitterRequest extends Request
{
    ...
    public function sanitize()
    {
        ...
        if ( !$this->has('dead_kits_amount')) {
            $input['dead_kits_amount'] = 0;
            $this->merge($input);
        }
    }
    ...
    public function rules()
    {
        if(!$this->get('breedplan')){
            $rules['bred']        = 'required|date_format:' . $this->user()->getDateFormatPHP();
            $rules['father_id']        = 'required';
            $rules['mother_id']        = 'required';
        }

        $rules['given_id']    = 'required';
        $rules['born']        = 'required|date_format:' . $this->user()->getDateFormatPHP();
