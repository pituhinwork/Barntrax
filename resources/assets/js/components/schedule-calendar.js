App.Components.ScheduleCalendar = {
    ...
    methods: {
        ...
        showAllTasksForRabbit: function (rabbit, type) {
            ...
            api.getTypeEvents(ntype, this.activeRabbit.id).then(data => {
                ...
                if (type == 'general') {
                    var temp = [];
                    data.forEach(function (item) {
                        if (item.type == 'general') temp.push(item);
                    });
                    ...
                    data.forEach(function (item) {
                        ...
                        $.each(recurringTasks, function (recurringTaskDate) {
                            if(recurringTaskDate == item.date){
                                recurringTasks[recurringTaskDate].forEach(function (recurringItem) {
                                    if(recurringTaskDate != item.date){
                    ...
                    $.each(recurringTasks, function (recurringTaskDate) {
                        recurringTasks[recurringTaskDate].forEach(function (item) {
                           self.activeTasks.push($.extend({}, item));
                        });
                    });
        ...
        showTasksForCurrentWeek: function (msg) {
            ...
            var type        = msg.type == 'general' ? 'users' : msg.type;

            api.getTypeEvents(type, type == 'users' ? null : this.activeRabbit.id, { weekStart: weekStart })
                .then(data => {
        ...
        parseWeeklyEvents: function (events, arrayData) {
            ...
            arrayData.forEach(function (itemData, dateIndex, arr) {
                ...
                events.forEach(function (itemEvent) {
        ...
        loadBreederTasks: function () {
            api.getEvents({ 'for': 'breeder', perPage: this.perPage }).then(data => {
                var temp = [];
        ...
        loadGeneralTasks: function(attachGeneralTasks, callbackFunction){
            api.getEvents({ 'for': 'user', perPage: this.perPage }).then(data => {
        ...
        loadLitterTasks: function () {
            api.getEvents({
                'for': 'litter', weekStart: App.formatDate(this.getFirstDayOfCurrentWeek()), perPage: this.perPage
            }).then(data => {
                var temp = [];
                data['data'].forEach(function (item) {
                    item.type = 'litters';
        ...
        openTaskForm: function (events, rabbit, type) {
            if (events.event) {
                if (type == "breeders" && events.event.subtype == "birth") {
                    return App.vent.openBirthModal(events.event.breed_id);
                } else if (type == "litters" && events.event.subtype == "butch") {
                    return App.vent.butcherModal(events.event.litter_id);
                } else if (type == "litters" && events.event.subtype == "weigh") {
                    return App.vent.weightModal(events.event.litter_id);
                }
            }
            if (events.count == 1) {
                ...
                this.$dispatch('schedule-calendar-all-task-of-current-week', {
                    rabbit: rabbit, events: events, type: type
                });
            }
        },
        ...
        getFirstDayOfCurrentWeek: function() {
            ...
            var n = Math.floor((delta + newYearDay)/7) - 1;
            var w = 1;
            var firstDay = newYear.getDay();
            var days = firstDay == w ? 1 : (n + 1) * 7 + w - (firstDay - 1);
            return new Date(currentYear, 0, days); //return date of Monday for current week.
        },
        ...
        lazyLoadTasks: function() {
            var self = this;
            api.getEvents({
                'for': 'all', 'perPage': this.perPage, 'skipBreeders': this.skipBreeders,
                'skipLitters': this.skipLitters
            }).then(data => {
                var fetchedAll;
                ...
                if (data['breeders']) {
                    ...
                    data['breeders'].forEach(function (item) {
                        if (item.events) {
                            item['type'] = 'breeders';
                ...
                if (data['litters']) {
                    countOfRabbits = 0;
                    fetchedAll = false;
                    data['litters'].forEach(function (item) {
                        if (item.events) {
                            item['type'] = 'litters';
                            self.calendar_events.push(item);
                        } else if (item == 'fetchedAll') {
        ...
        scrollHandlerLazyLoad: function() {
        ...
        eventTitle: function(event) {
            if(event.name && event.recurring_events[0] && event.recurring_events[0].date) {
                return event.name +': '+ event.recurring_events[0].date;
            }
